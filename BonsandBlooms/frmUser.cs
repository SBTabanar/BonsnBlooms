...
namespace BonsandBlooms
{
    public partial class frmUser : Form
    {
        ...
        {
            string newText = new string(txtname.Text.Where(c => !char.IsDigit(c)).ToArray());
            if (newText != txtname.Text)
            {
                int selStart = txtname.SelectionStart - 1;
                txtname.Text = newText;
                if (selStart >= 0)
                    txtname.SelectionStart = selStart;
            }
        }
